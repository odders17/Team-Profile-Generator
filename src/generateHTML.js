...
// MyTeam HTML Generator
const generate = (teamArr) => {
    // Functions to Create New Arrays of Employee Card HTML, Arranging Employees by Position
    ...
    // Arrays to Hold HTML for Each Employee Card by position
    let managerCardsHTML = [];
    let engineerCardsHTML = [];
    let internCardsHTML = [];

    // Arrays of Employee Cards Generator
    genManagers(teamArr);
...
const generateHTML = (teamArr) => {
    fs.writeFile('./placement/MyTeam.html', generate(teamArr), function(err, result) {
        (err) ? console.log('error', err): console.log(`\r\nSuccessfully wrote MyTeam.html to your placement folder.`.underline.green);
        }
    );
};
